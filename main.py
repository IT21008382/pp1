...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        ...
        for face_landmarks in results.multi_face_landmarks:
            ...
            # # Calculate attention metric if enough time has elapsed
            # if elapsed_time >= attention_duration:
            #     directions_array = np.array(face_directions)
            #     direction_counts = np.zeros_like(directions_array)

            #     for i, dir in enumerate(directions_array):
            #         direction_counts[i] = np.sum(
            #             np.isclose(directions_array, dir, atol=10.0).all(axis=1))

            #     attention_score = np.max(direction_counts) / len(face_directions)
            ...
            #     cv2.putText(image, attention_status, (400, 400),
            #                 cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)

            #     # Reset tracking after calculating attention status
            #     face_directions = []
            #     start_time = time.time()
